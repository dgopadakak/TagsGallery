...
class FullscreenTagsEditorTest {
    ...
    fun `opening editor loads tag ids already linked to the media`() = runTest(testDispatcher) {
        ...
        assertEquals(
            ...
            viewModel.uiState.value.tagsEditor?.selectedTagIds
        )
    }
    ...
    fun `saving closes the editor`() = runTest(testDispatcher) {
        ...
        assertNull(viewModel.uiState.value.tagsEditor)
    }
    ...
    fun `saving an empty selection asks to confirm media removal`() = runTest(testDispatcher) {
        ...
        assertEquals(true, viewModel.uiState.value.tagsEditor?.confirmingRemoval)
    }

    @Test
    fun `dismissing the removal confirmation returns to the editor with selection kept`() =
        runTest(testDispatcher) {
            openEditorForFirstMedia(linkedTagIds = listOf(SUMMER_TAG_ID, FRIENDS_TAG_ID))
            ...
            assertEquals(
                MainViewModel.TagsEditorState(
                    mediaUri = FIRST_MEDIA_ID.toUri(),
                    selectedTagIds = emptySet(),
                    confirmingRemoval = false
                ),
                viewModel.uiState.value.tagsEditor
            )
        }
    ...
    fun `tags are exposed to the editor sorted by name`() = runTest(testDispatcher) {
        ...
        assertEquals(
            ...
            viewModel.uiState.value.allTags.map { it.name }
        )
    }
    ...
    /**
     * Открывает редактор для первого медиа. Второе медиа с тегом Sea есть в базе всегда - это
     * контрольные связи, которых правки первого медиа касаться не должны
     */
    private suspend fun TestScope.openEditorForFirstMedia(linkedTagIds: List<Long>) {
        insertTags()
